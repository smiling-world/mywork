...
class Buffer:
    ...
    def get_data(self, size: int, transform: nn.Module = None, return_index=False, classes = None, positive = True) -> Tuple:
        """
        ...
        """
        # if positive:
        #     mask = torch.zeros(self.examples.shape[0]).bool().to(self.device)
        # else:
        #     mask = torch.ones(self.examples.shape[0]).bool().to(self.device)
        # for c in classes:
        #     if positive:
        #         mask = mask | (self.labels == c)
        #     else:
        #         mask = mask & (~(self.labels == c))
        #
        # data = self.examples[mask]
        # if size > min(self.num_seen_examples, data.shape[0]):
        #     size = min(self.num_seen_examples, data.shape[0])
        #
        # choice = np.random.choice(min(self.num_seen_examples, data.shape[0]),
        ...
        # if size > 0:
        #     ret_tuple = (torch.stack([transform(ee.cpu()) for ee in data[choice]]).to(self.device),)
        # else:
        #     ret_tuple = (torch.zeros(0).to(self.device), )
